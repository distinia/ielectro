CREATE TABLE IF NOT EXISTS `dominions_currencies` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_dominions_currencies_name` (`name`),
  UNIQUE KEY `uq_dominions_currencies_code` (`code`)
);
CREATE TABLE IF NOT EXISTS `dominions_territory_economies` (
  ...
  CONSTRAINT `dominions_territory_economies_ibfk_2`
    ...
    REFERENCES `dominions_currencies` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE
);
CREATE TABLE IF NOT EXISTS `dominions_resources` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(100) NOT NULL,
  `description` TEXT DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_dominions_resources_name` (`name`)
);
CREATE TABLE IF NOT EXISTS `dominions_territory_resources` (
  `territory_id` BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT `dominions_territory_resources_ibfk_2`
    ...
    REFERENCES `dominions_resources` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
);
CREATE TABLE IF NOT EXISTS `dominions_industries` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(100) NOT NULL,
  `description` TEXT DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_dominions_industries_name` (`name`)
);
CREATE TABLE IF NOT EXISTS `dominions_territory_industries` (
  `territory_id` BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT `dominions_territory_industries_ibfk_2`
    ...
    REFERENCES `dominions_industries` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
);