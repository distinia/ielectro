...
class Templates
{
    ...
    {
        $row = Query::fetch(
            "SELECT id FROM ielectro_dyscover.dyscover_posts
    ...
    {
        $row = Query::fetch(
            "SELECT user_id FROM ielectro_dyscover.dyscover_posts
            WHERE id = ? AND type = 'template' AND status = 'active' LIMIT 1",
            [$id]
        );
        if (!$row) {
...
class TemplateFields
{
    ...
    public static function sync(int $templateId, mixed $fields): void
    {
        ...
        $existing = Query::fetchAll(
            'SELECT id FROM ielectro_dyscover.dyscover_template_fields WHERE template_id = ?',
            [$templateId]
        );
        ...
        foreach ($fields as $position => $field) {
            ...
            Query::execute(
                'INSERT INTO ielectro_dyscover.dyscover_template_fields(template_id, name, type, position)
                VALUES (?, ?, ?, ?)',
        ...
        foreach (array_diff($existingIds, $keptIds) as $removeId) {
            Query::execute(
                'DELETE FROM ielectro_dyscover.dyscover_template_fields
                WHERE id = ? AND template_id = ?',
                [$removeId, $templateId]
            );
        }
